...
from .graphics.shading import ShaderRender, ShaderManager
from .entity import EntityManager, Draw
from .inputting import Input
from typing import cast, Callable

class WindowManager:
    _window_processes: list[multiprocessing.Process] = []

    @classmethod
    def run_window(cls, loop: Callable[..., None]):
...
class Window:
    ...
    def gameloop(self):
        ...
        while not self.wnd.is_closing:
            ...
            alpha = min(accumulator / DELTA_TIME, 1)
            self.render(alpha)
    ...
    def run(self):
        WindowManager.run_window(self.gameloop)

    def update(self, dt: float):
        CameraManager.update_camera_lerp()
        EntityManager.update_transform_lerp()
        ...
        EntityManager.update_transforms()
...
class GameLoop:
    ...
    _screen_size: tuple[int, int] = (800, 600)
