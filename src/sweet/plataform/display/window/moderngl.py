...
class GLWindow(WindowSurface):
    ...
    def initialize(self, width: int, height: int, title: str):
        ...
        if GLWindow._PRIMARY_WINDOW_HANDLE is None:
            raise RuntimeError(
                "Engine em OpenGL falhou" "Engine precisa inicializar contexto dummy"
            )
        ...
        try:
            ...
            if primary_handle is None and hasattr(
                self.graphics_device, "_dummy_window"
            ):
    ...
    def poll_events(self) -> bool:
        ...
        if handle:
            ...
            if (current_w, current_h) != self._size and current_w > 0 and current_h > 0:
                self._apply_size(current_w, current_h)
    ...
    def swap_buffers(self):
        if self.wnd and self._offscreen_target:
            ...
            self.graphics_device.blit_texture_to_target(
                src_target=self._offscreen_target, dst_target=self._backbuffer_target
            )
