...
class ModernGLRenderPipeline(RenderPipeline):
    def __init__(self, descriptor: RenderPipelineDescriptor):
        ...
        self._vao_cache: dict[tuple[int, int, Optional[int], int], moderngl.VertexArray] = {}
...
class ModernGLCommandBuffer(CommandBuffer):
    ...
    def begin_render_pass(self, target: RenderTarget, viewport: Optional[tuple[int, int, int, int]] = None, clear_color: tuple[float, float, float] = (0.1, 0.2, 0.3)) -> None:
        ...
        def cmd_begin_pass(t: RenderTarget=target, vp: Optional[tuple[int, int, int, int]]=viewport, cc: tuple[float, float, float]=clear_color):
    ...
    def draw_indexed(
        ...
        p, v, i, t = self._current_pipeline, self._current_vbo, self._current_ibo, self._current_target
...
class ModernGLGraphicsDevice(GraphicsDevice):
    ...
    def create_framebuffer(self, width: int, height: int) -> RenderTarget:
        if not self.ctx:
            raise RuntimeError("Inicialize o contexto ModernGL antes de criar um framebuffer")
    ...
    def create_command_buffer(self) -> CommandBuffer:
        if not self.ctx:
            raise RuntimeError("Inicialize o contexto ModernGL antes de criar um command buffer")
